"""Generate a multi-episode LeRobot dataset with the RealSense IK pick-and-place scene.
...
contains seven arm joints plus one gripper value and defaults to radians.

Usage:
    /home/zxro/IsaacLab/isaaclab.sh -p \
        /home/zxro/smolVLA-isaacLab/scripts/openarm_table_realsense_ik_pick_place_make_dataset.py \
        --num_episodes 10
"""
...
parser.add_argument(
    ...
    choices=("rad", "degree"),
    default="rad",
    help="Unit used for observation.state and action; defaults to rad.",
)
...
parser.add_argument(
    ...
    help="Recorded RGB width; defaults to 640 pixels.",
)
parser.add_argument(
    "--camera_height",
    type=int,
    default=DEFAULT_CAMERA_HEIGHT,
    help="Recorded RGB height; defaults to 480 pixels.",
)
...
if args_cli.dataset_fps < 1:
    parser.error("--dataset_fps must be at least 1")
if args_cli.camera_width < 1 or args_cli.camera_height < 1:
    parser.error("--camera_width and --camera_height must be positive")
...
class PickPlaceSceneCfg(InteractiveSceneCfg):
    ...
    realsense_color = CameraCfg(
        prim_path="{ENV_REGEX_NS}/RealsenseColorCamera",
        update_period=0.0,
...
def create_realsense_view(scene: InteractiveScene):
    """Match the recording camera to the original USD color camera and display it."""
    stage = get_current_stage()
    realsense_root = f"{scene.env_prim_paths[0]}/Realsense"
    source_paths = [
        prim.GetPath()
        for prim in Usd.PrimRange(stage.GetPrimAtPath(realsense_root))
    ...
    # CameraCfg needs its own camera prim for RGB tensor capture. Copy the
    # original RealSense USD camera's complete world pose and optical settings
    # onto that recording prim so its rendered image has the original viewport
    # framing rather than the RealSense body-origin framing.
    record_path = Sdf.Path(f"{scene.env_prim_paths[0]}/RealsenseColorCamera")
    ...
    print(f"[INFO] Dataset RGB camera matched to original viewport camera: {source_path}")
    view = create_viewport_window(
        name="RealSense D455 Color",
        width=args_cli.camera_width,
        height=args_cli.camera_height,
        camera_path=source_path,
    )
    scene_view = ui.Workspace.get_window("Viewport")
    if view is not None and scene_view is not None:
        view.dock_in(scene_view, ui.DockPosition.RIGHT, 0.5)
    return view
...
class PickPlaceController:
    ...
    def get_record_state_action(self) -> tuple[np.ndarray, np.ndarray]:
        # Isaac Lab stores all revolute joints in radians. The dataset layout
        # is [joint_1, ..., joint_7, gripper], with the output unit selected by
        # --joint_unit (rad by default).
        state_rad = torch.cat(
            (
        ...
        if args_cli.joint_unit == "degree":
            state_tensor = torch.rad2deg(state_rad)
            action_tensor = torch.rad2deg(action_rad)
        else:
            state_tensor = state_rad
            action_tensor = action_rad
        state = state_tensor.detach().cpu().numpy().astype(np.float32)
    ...
    def advance(self, dt: float) -> None:
        ...
        if self.state == "wait_settle":
            ...
            self.set_gripper(True)
            if self.interpolated_move(args_cli.move_time_s):
            ...
            self.set_gripper(True)
            # Rotate only during the final descent, reaching 45 degrees at the cube center.
...
class MultiEpisodeLeRobotRecorder:
    ...
    def __init__(self, controller: PickPlaceController, rgb_sensor) -> None:
        ...
        if args_cli.record_camera:
            features["observation.images.top"] = {
                ...
                "shape": (args_cli.camera_height, args_cli.camera_width, 3),
                "names": ["height", "width", "channels"],
            }
            self.camera = IsaacLabRGBSensorGrabber(
                rgb_sensor, args_cli.camera_width, args_cli.camera_height
            )
        ...
        print(f"[RECORD] Recording {args_cli.num_episodes} episode(s) at {args_cli.dataset_fps} FPS")
        print(f"[RECORD] Joint state/action unit: {args_cli.joint_unit}")
        print(f"[RECORD] RGB resolution: {args_cli.camera_width}x{args_cli.camera_height}")
    ...
    def record_if_needed(self, dt: float, force: bool = False) -> None:
        self.elapsed_s += dt
        period = 1.0 / float(args_cli.dataset_fps)
        if force or self.elapsed_s + 1e-9 >= self.next_frame_t:
            ...
            if self.camera is not None:
                frame["observation.images.top"] = self.camera.read()
...
def run_simulator(sim: SimulationContext, scene: InteractiveScene, rgb_sensor) -> None:
    ...
    try:
        for episode_index in range(args_cli.num_episodes):
            if not simulation_app.is_running():
                break
            print(f"\n[EPISODE] Starting {episode_index + 1}/{args_cli.num_episodes}")
            reset_scene(scene)
            ...
            if recorder is None:
                recorder = MultiEpisodeLeRobotRecorder(controller, rgb_sensor)
            else:
                recorder.begin_episode(controller)

            while simulation_app.is_running() and controller.state != "done":
...
def main() -> None:
    ...
    scene = InteractiveScene(PickPlaceSceneCfg(num_envs=1, env_spacing=1.0))
    sim.reset()
    create_realsense_view(scene)
    rgb_sensor = scene["realsense_color"] if args_cli.record_camera else None
    print("[INFO] OpenArm tilted IK pick-and-place dataset scene ready.")
    print(f"[INFO] Cube drop xyz={CUBE_POS}; storage center xyz={STORAGE_BOX_POS}")
    run_simulator(sim, scene, rgb_sensor)
